...
class SessionMaker extends \SessionHandler
{
    ...
    public function __construct($key, $name = '_scriptlog', $cookie = [])
    {
        ...
        if (PHP_VERSION_ID < 70300) {
            ...
            // 'session.cookies_path' is an app-specific directive that may be
            // unset; fall back to PHP's real session.cookie_path ('/' by
            // default). An empty path makes browsers scope the session cookie
            // to the current request directory (e.g. /api/v1/posts/123/), so
            // sessions created by deep-path endpoints never reach the frontend.
            $cookiesPath = ini_get('session.cookies_path');

            if ($cookiesPath === false || $cookiesPath === '') {
                $cookiesPath = ini_get('session.cookie_path');
