...
function theme_identifier()
{
    ...
    if (!empty($loaded) && is_array($loaded)) {
        $loaded = premium_theme_license_supports_render($loaded);
    }
...
/**
 * premium_theme_license_supports_render()
 *
 * If the active theme is a premium theme whose license is not currently
 * active, fall back to the built-in 'blog' theme so the site does not
 * render a premium theme the owner is not licensed to use. This is a
 * local check only - no API call is made on every request.
 *
 * @category functions
 * @param array $theme the active theme row (from tbl_themes)
 * @return array the theme row to actually render
 *
 */
function premium_theme_license_supports_render($theme)
{
    $isPremium = (isset($theme['theme_type']) && $theme['theme_type'] === 'premium');

    if (!$isPremium && !empty($theme['theme_directory'])) {
        $iniFile = APP_ROOT . APP_THEME . basename($theme['theme_directory']) . DIRECTORY_SEPARATOR . 'theme.ini';
        if (file_exists($iniFile)) {
            $ini = parse_ini_file($iniFile, true);
            if (is_array($ini) && isset($ini['info']['theme_type']) && $ini['info']['theme_type'] === 'premium') {
                $isPremium = true;
            }
        }
    }
    ...
    $licenseActive = (
        isset($theme['license_status'])
        && $theme['license_status'] === 'active'
    );

    if ($licenseActive && !empty($theme['license_expires_at'])) {
        $expires = strtotime((string)$theme['license_expires_at']);
    ...
    $fallback = $themeDao->findThemeByDirectory('blog');

    if (!empty($fallback) && is_array($fallback)
        && isset($fallback['theme_directory'])) {
        $fallback['theme_type'] = isset($fallback['theme_type']) ? $fallback['theme_type'] : 'free';
        $fallback['license_status'] = isset($fallback['license_status']) ? $fallback['license_status'] : 'active';
        return $fallback;
    }
