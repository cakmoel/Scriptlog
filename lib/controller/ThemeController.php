...
class ThemeController extends BaseApp
{
    ...
    private function retrieveSessionErrors()
    {
        ...
        if (isset($_SESSION['error'])) {
            ...
            ($_SESSION['error'] == 'themeLicenseError') ? array_push($errors, isset($_SESSION['error_message']) ? "License Error: " . $_SESSION['error_message'] : "License Error: Unable to activate this premium theme.") : "";
    ...
    public function enableTheme($id)
    {
        ...
        if (isset($_GET['Id'])) {
            ...
            try {
                ...
                $licenseKey = isset($_GET['license_key'])
                    ? trim((string)$_GET['license_key'])
                    : null;

                if (isset($licenseKey) && $licenseKey !== '') {
                    $licenseKey = Sanitize::severeSanitizer($licenseKey);
                }

                $this->themeService->setThemeId($id);
                $activation = $this->themeService->activateInstalledThemeWithLicense($licenseKey);

                if (empty($activation['success'])) {
                    $_SESSION['error'] = 'themeLicenseError';
                    $_SESSION['error_message'] = $activation['message'];
                    direct_page('index.php?load=templates&error=themeLicenseError', 302);
                }
