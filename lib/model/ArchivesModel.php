...
class ArchivesModel extends BaseModel
{
    ...
    public function getArchivesOnSidebar()
    {
        ...
        $sql = "SELECT YEAR(tbl_posts.post_date) AS year_archive, 
            ...
            FROM tbl_posts 
            WHERE tbl_posts.post_type = 'blog' AND tbl_posts.post_status = 'publish'
     ...
     * getArchiveIndex
     *
     * retrieves all archives with post counts for index page
     * @return mixed
     *
     */
    public function getArchiveIndex()
    {
        ...
        $sql = "SELECT YEAR(tbl_posts.post_date) AS year_archive, 
            ...
            WHERE tbl_posts.post_type = 'blog' AND tbl_posts.post_status = 'publish'
            GROUP BY year_archive, month_archive
            ORDER BY year_archive DESC, month_archive DESC";
    ...
    {
        $sql = "SELECT ID FROM tbl_posts WHERE MONTH(post_date) = ? AND YEAR(post_date) = ? AND post_visibility = 'public' ";
        $this->setSQL($sql);
