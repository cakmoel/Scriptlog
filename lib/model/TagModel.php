...
class TagModel extends BaseModel
{
    ...
    public function getLinkTag($postId, $sanitize)
    {
        ...
        // post_tags is nullable; strtolower(null) raises a deprecation on
        // PHP 8.x that escalates to a 500 under strict error handlers.
        $tag_exploaded = (empty($tags)) ? [] : explode(',', strtolower($tags));
    ...
    public function getPostsPublishedByTag($tag, $sanitize, Paginator $perPage)
    {
        ...
        $count_tags = "SELECT tbl_posts.ID, tbl_posts.post_title, tbl_posts.post_content, tbl_posts.post_tags 
                 FROM tbl_posts WHERE tbl_posts.post_tags LIKE :post_tags AND tbl_posts.post_visibility = 'public' ORDER BY tbl_posts.ID DESC";
        ...
        $sql = "SELECT tbl_posts.ID, tbl_posts.media_id, tbl_posts.post_author, 
           ...
           AND tbl_posts.post_visibility = 'public'
