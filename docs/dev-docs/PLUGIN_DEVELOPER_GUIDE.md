...
2. `PluginService::activateInstalledPlugin()`:
   - Executes `schema.sql` if exists (for database tables, split on `;`, skipping `--` comments)
   - Calls `enable_plugin($plugin_path)` to load PHP files via `require_once` (loads main class file and `functions.php`)
   - Updates `plugin_status` = 'Y' in database
3. The plugin's PHP class file is loaded, but hooks are **not yet registered** — they are registered later when the class is instantiated during `invoke_plugin()` call
...
3. Plugin files remain on disk, database record preserved
4. Hooks are not invoked while disabled (plugin not found by `invoke_plugin()`)
...
3. **Note:** The `uninstall()` method in your plugin class is a convention method — it is **not called by the framework** during deletion. Handle cleanup via `schema.sql` DROP statements or hooks instead.
...
| plugin_action | Yes | Action identifier for routing (used with plugin_loader to generate plugin_link) |

**Note:** `plugin_link` is auto-generated from `plugin_loader` and `plugin_action` during installation via `generate_request()` and is not set in `plugin.ini`.

**Note on `activate()`, `deactivate()`, `uninstall()`:** These are **convention methods** defined in the plugin class template. The framework does **not** call them during activation, deactivation, or deletion. They are included for documentation purposes and can be invoked manually by the developer if needed. Use `clip()` hooks for runtime behavior instead.
...
class MyPlugin
{
    ...
    {
        // Convention method — not called by the framework
        return true;
    }
...
**Note:** `functions.php` is automatically loaded by `enable_plugin()` during plugin activation via `require_once` when it exists.

### Step 4: Create Database Schema (Optional)
If your plugin needs database tables, create `schema.sql`. During activation, the framework processes this file by splitting on `;`, skipping lines starting with `--`, and executing each SQL statement.
...
| Plugin loads but no output | Ensure class is instantiated during `invoke_plugin()` and hooks registered in `registerHooks()` |
...
2. **Always** use unique hook names with your plugin prefix (e.g., `clip_myplugin_`) to avoid conflicts
3. **Always** sanitize user input in your plugin
4. **Avoid** modifying core files - use hooks (`clip()`) instead
5. **Document** your plugin's hooks, functions, and configuration options
6. **Test** on a development environment first
7. **Version** your plugin using semantic versioning
8. **Remember** that `activate()`, `deactivate()`, and `uninstall()` are convention methods — they are **not called by the framework**. Use `clip()` hooks for runtime behavior.
