...
- `lib/autoload-aliases.php` - 233 `class_alias()` entries mapping old global names to new namespaced classes (kept as reference)
- `lib/autoload-aliases-map.php` - Static array map (233 entries) used by the lazy autoloader (fast, no class loading)
...
This script performed the **one-time migration** to add `namespace Scriptlog\*` declarations to all 179 original class files, add cross-namespace `use` imports where needed, and generate backward-compatible `class_alias()` entries so existing code continued working. Since then the project has grown to 233 classes, each new one being added manually to both alias files (see the TIP below).
...
| **3: Generate aliases** | Writes `lib/autoload-aliases.php` with `class_alias()` entries mapping old global names (e.g., `Bootstrap`) to new namespaced names (e.g., `Scriptlog\Core\Bootstrap`) | `lib/autoload-aliases.php` (created; now 233 entries) |
...
|   |-- autoload-aliases.php   # 233 class_alias() entries (reference only)
|   |-- autoload-aliases-map.php # Static alias map for lazy autoloader (233 entries)
|   |
...
|       +-- ...                # 19 DAOs total (see docs/dev-docs/DATABASE_SCHEMA_GUIDE.md)
|
...
|           +-- TopicApiDto.php
|
|   +-- service/               # Business logic layer - Scriptlog\Service (24 files)
|       |-- PostService.php
...
The database is MySQL/MariaDB with InnoDB tables. The schema is **23 tables**. Every definition, index, default value, and seed insert lives in one file:
...
| Feature | Implementation |
|---------|---------------|
| **XSS Prevention** | Server-side sanitization via `SearchFinder::sanitizeKeyword()` + `theme_escape_html()` in template + `escapeHtml()` on all widget fields with reflected keyword/count escaped before HTML interpolation |
| **SQL Injection** | Uses PDO prepared statements |
| **Bot Protection** | `search_verify` honeypot field on the HTML form plus 30 req/60 s per IP rate limiting on `SearchController` and `SearchApiController` (GET search forms carry no CSRF token) |
...
- **API rate limiting**: 30 req/60 s per IP (`search_api` namespace); 429 responses render the `search.rate_limited` string in the widget
- **Autocomplete contract**: typing renders up to 10 inline suggestions; explicit submit navigates to the full page URL built from `scriptlog_vars.search_url` (permalink aware) so the GET fallback works with JS on or off
- **Speed**: 300 ms debounce, abort of the in flight XHR with stale out of order responses dropped, 5 minute in memory cache for repeat queries
- **Keyboard and roles**: input is `combobox`, dropdown is `listbox` with `option` items; ArrowUp/ArrowDown move the highlight, Enter opens the highlighted hit, Escape dismisses
...
2. **Include the sidebar search form** (typically in `sidebar.php`). Never hard code `/search`; use `theme_search_url()` so the action matches the active permalink scheme. GET search forms carry no CSRF token; bot protection is the `search_verify` honeypot:

```php
<form action="<?= theme_escape_html(theme_search_url()); ?>" method="get" class="search-form" id="ajax-search-form"
      role="search">
    ...
    <div class="hp-field" style="display:none" aria-hidden="true"><input type="text" name="search_verify" autocomplete="off" tabindex="-1" value=""></div>
</form>
```
