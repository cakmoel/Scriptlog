...
Widgets include:
- **Search form**: AJAX autocomplete while typing plus full page GET on submit (no CSRF token on GET; bot protection is a `search_verify` honeypot), `aria-live` results container with `listbox`/`option` roles, a clear button, keyboard support (ArrowUp/ArrowDown/Enter/Escape), and a permalink-aware non-JS fallback (the `search_action` is built with `theme_search_url()` - `/search` when permalinks are ON, app root `?q=` when OFF)
...
```php
// Search widget essentials - note the honeypot (no CSRF token on GET) and aria-live containers
<form action="<?= theme_escape_html($search_action); ?>" method="get" class="search-form" id="ajax-search-form"
    ...
    <input type="search" id="search-keyword" name="q" class="search-input"
           placeholder="<?= t('sidebar.search.placeholder'); ?>" autocomplete="off" autocapitalize="none" spellcheck="false" maxlength="100"
    ...
    <div id="search-results" class="search-results" role="listbox" aria-live="polite" aria-atomic="true"></div>
...
4. The AJAX request sends only `q` and `type=all` (no CSRF token on GET; the `search_verify` honeypot lives only in the plain HTML form fallback, and a 429 response renders the `search.rate_limited` string)
...
**Autocomplete contract (typing vs submit):** typing renders up to 10 inline suggestions in the dropdown; explicit submit (Enter with no highlighted hit, or the submit button) navigates to the full page URL built from `scriptlog_vars.search_url`, so the GET fallback keeps working with JS enabled. Requests use a 300 ms debounce, abort the in flight XHR and drop stale out of order responses, cache repeat queries for 5 minutes, and escape the reflected keyword and counts before HTML interpolation. Keyboard: ArrowUp/ArrowDown move the highlight, Enter opens the highlighted hit, Escape dismisses.
...
| `block_csrf()` | `(): string` | Generate CSRF token for state changing forms such as comments (search forms use GET and carry no CSRF token) | Hidden input HTML |
...
| `search.widget.*` | Sidebar widget: count, no_results, view_all, loading, error (plus `search.rate_limited` for 429 responses, exposed to JS via `header.php`) |
...
- [ ] State changing forms (comment, login) include a CSRF token via `block_csrf()` (search forms use GET and carry no CSRF token; bot protection is the `search_verify` honeypot)
