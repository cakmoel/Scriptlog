...
> These totals are a snapshot from the **last recorded run (August 2026)** - rerun the suite to refresh them.
...
> The phase table above is a historical record of when tests were added. Last recorded run (August 2026): 2,269 tests across 167 files with 8,074 assertions (15 skipped, 60 risky). These totals are a snapshot - rerun the suite to refresh them.
...
CI runs without any database or MySQL service. PHP unit tests and static
analysis run in `.github/workflows/tests.yml`; end-to-end browser tests run
via Playwright in `.github/workflows/playwright.yml`. Database-backed
Integration/Service suites are local-only (see §6).
  ...
  test:
    ...
    steps:
      ...
      - name: Run Unit Tests (no database required)
        run: lib/vendor/bin/phpunit --configuration phpunit.xml --testsuite "Unit Tests" --no-coverage --colors=never

      - name: Run PHPStan
        run: composer phpstan
```

End-to-end tests run separately via Playwright (`.github/workflows/playwright.yml`):
