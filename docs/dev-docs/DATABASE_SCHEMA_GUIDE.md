...
**Version:** 1.8.1 | **Last Updated:** September 2026
...
> Note: the installer creates all 23 tables, including `tbl_theme_licenses`. Installations created before that table was added to the installer get it by running `install/migrate-theme-license.php`.
...
Scriptlog has 23 tables. The list below is the complete set.
...
| 17 | `tbl_theme_licenses` | Premium theme license activations |
| 18 | `tbl_consents` | GDPR cookie and consent records |
| 19 | `tbl_data_requests` | GDPR data access and erasure requests |
| 20 | `tbl_privacy_logs` | Audit log of privacy actions |
| 21 | `tbl_privacy_policies` | Localized privacy policy content |
| 22 | `tbl_languages` | Supported languages |
| 23 | `tbl_translations` | UI translation strings per language |
...
Indexes: PRIMARY KEY (`ID`), KEY `author_id` (`post_author`), KEY `post_media` (`media_id`), KEY `idx_post_slug` (`post_slug`), KEY `idx_post_locale` (`post_locale`), KEY `idx_post_status_date` (`post_status`, `post_date`), FULLTEXT KEY (`post_tags`, `post_title`, `post_content`).

The FULLTEXT index powers the search engine. The slug index supports canonical URL validation, and the composite status/date index speeds up ordered listings of published content.
...
| `theme_type` | ENUM('free','premium') NOT NULL DEFAULT 'free' | Distribution type |
| `license_key` | VARCHAR(100) DEFAULT NULL | License key, for premium themes |
| `license_status` | ENUM('active','inactive','expired','invalid') DEFAULT NULL | Current license state |
| `license_expires_at` | DATETIME DEFAULT NULL | License expiry, when time-limited |

Indexes: PRIMARY KEY (`ID`), KEY `theme_directory` (`theme_directory`), KEY `license_key` (`license_key`).

### tbl_theme_licenses

Activation records for premium themes. When a premium theme is activated, a row is written here so the license can be verified and tied to the installation domain.

| Column | Attributes | Description |
|--------|------------|-------------|
| `ID` | BIGINT(20) UNSIGNED NOT NULL AUTO_INCREMENT, PRIMARY KEY | License record identifier |
| `theme_id` | INT(11) UNSIGNED NOT NULL | References `tbl_themes.ID` |
| `license_key` | VARCHAR(100) NOT NULL | License key issued for the theme |
| `license_status` | VARCHAR(20) NOT NULL DEFAULT 'active' | `active`, `inactive`, `expired`, or `invalid` |
| `domain` | VARCHAR(255) NOT NULL | Domain the license was activated on |
| `remote_license_id` | INT(11) DEFAULT NULL | Identifier returned by the remote license server |
| `activated_at` | TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP | Activation time |
| `expires_at` | DATETIME DEFAULT NULL | Expiry, when time-limited |

Indexes: PRIMARY KEY (`ID`), KEY `theme_id` (`theme_id`), KEY `license_key` (`license_key`), KEY `domain` (`domain`).

The link to `tbl_themes` is application-managed. Fresh installations create this table from the installer; existing installations create it with `install/migrate-theme-license.php`, which also normalizes the columns, indexes, and collation of a table created by an earlier version to match the definition above.
...
| `ThemeDao` | `tbl_themes`, `tbl_theme_licenses` |
