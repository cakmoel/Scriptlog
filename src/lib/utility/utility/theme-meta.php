...
function metatag_by_query($key, $value, $scriptlog_image, $scriptlog_imgthumb)
{
  ...
  switch ($key) {
    ...
    case 'tag':
      ...
      if (!empty($value)) {
        ...
        if (is_iterable($tags)) {
          ...
          foreach ($tags as $tag) {
            ...
            $tag_title = isset($tag['post_tags']) ? explode(", ", escape_html($tag['post_tags'])) : null;
            $description = isset($tag['post_content']) ? escape_html($tag['post_content']) : null;
            $keyword = isset($tag['post_summary']) ? escape_html($tag['post_summary']) : null;
          }
      ...
      $theme_meta['site_schema'] = (is_null($tag_id)) ? generate_schema_org(ucfirst(trim('page not found')) . '' . app_info()['site_name']) : generate_schema_org(strtolower(trim($value)), $canonical, $scriptlog_image, $description, $tag_title, $scriptlog_imgthumb, date(DATE_ATOM));
