...
function get_user_signin($user_login)
{
    ...
    $result = db_prepared_query($sql, [$user_login], "s");
    return fetch_assoc($result);
}
...
function alert_login_attempt($ip)
{
    if (filter_var($ip, FILTER_VALIDATE_IP)) {
        $sql = "SELECT count(ip_address) AS alert_login_attempt 
           FROM tbl_login_attempt 
           WHERE ip_address = ? 
           AND login_date >= DATE_SUB(NOW(), INTERVAL 1 DAY)";
...
/**
 * fetch_assoc - Helper to get associative array from result (PDO/mysqli compatible)
 */
