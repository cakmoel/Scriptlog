...
function theme_meta()
{
    ...
    // Check permalink setting - handle empty/null values properly
    $permalinkEnabled = function_exists('is_permalink_enabled') ? is_permalink_enabled() : 'no';
    if (empty($permalinkEnabled) || $permalinkEnabled !== 'yes') {
        $permalinkEnabled = 'no';
    }
...
function metatag_by_path($scriptlog_image, $scriptlog_imgthumb, $uri)
{
    ...
    switch ($uri->matched) {
        case 'post':
            ...
            $theme_meta['site_schema'] = is_null($post_id) ? generate_schema_org(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : BlogSchema::generateBlogPostSchema($post_id);
            $theme_meta['site_meta_tags'] = is_null($post_id) ? generate_meta_tags(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_meta_tags(ucfirst(trim($post_title)), $description, $author, $image, $canonical, true);
        ...
        case 'page':
            if (!empty($uri->param1)) {
                ...
                if (!empty($read_page) && is_array($read_page)) {
                    ...
                    $author = (!empty($read_page['user_login'])) ? escape_html($read_page['user_login']) : escape_html(app_info()['site_name']);
            ...
            $theme_meta['site_schema'] = is_null($page_id) ? generate_schema_org(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_schema_org(ucfirst(trim($page_title)) . ' | ' . escape_html(app_info()['site_name']), $canonical, $image, escape_html(app_info()['site_keywords']), $description, $image, $date_published, $date_modified);
            $theme_meta['site_meta_tags'] = is_null($page_id) ? generate_meta_tags(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_meta_tags($page_title, $description, $author, $image, $canonical);
        ...
        case 'blog':
            ...
            $theme_meta['site_schema'] = generate_schema_org(app_info()['site_name'] . ' | ' . app_info()['site_tagline'], app_url(), $scriptlog_image, escape_html(app_info()['site_description']), escape_html(app_info()['site_keywords']), $scriptlog_imgthumb, $date_published, $date_modified);
            $theme_meta['site_meta_tags'] = generate_meta_tags('Blog: ' . ucfirst(trim(app_info()['site_description'])), escape_html(app_info()['site_keywords']), escape_html(app_info()['site_name']), $scriptlog_image, app_url() . DS . 'blog');
        ...
        case 'category':
            ...
            $theme_meta['site_schema'] = (is_null($topic_id)) ? generate_schema_org(ucfirst(trim('page not fiund')) . ' | ' . escape_html(app_info()['site_name'])) : generate_schema_org(ucfirst(trim($topic_title)), $canonical, $scriptlog_image, $description, escape_html(app_info()['site_keywords']), $scriptlog_imgthumb, $date_published, $date_modified);
            $theme_meta['site_meta_tags'] = (is_null($topic_id)) ? generate_meta_tags(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_meta_tags('Category: ' . ucfirst(trim($topic_title)), $description, escape_html(app_info()['site_name']), $scriptlog_image, $canonical);
        ...
        case 'archive':
            ...
            $description = 'Archive: ' . $month_name  . ' | ' . escape_html(app_info()['site_description']);
            ...
            $theme_meta['site_meta_tags'] = (is_null($month)) ? generate_meta_tags(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_meta_tags('Archive: ' .  ucfirst(trim($month_name)), $description, escape_html(app_info()['site_name']), $scriptlog_image, $canonical);
        ...
        case 'tag':
            ...
            $theme_meta['site_schema'] = (is_null($tag_item)) ? generate_schema_org(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_schema_org(ucfirst(trim(escape_html($tag_item))) . ' | ' . escape_html(app_info()['site_name']), $canonical, $scriptlog_image, $description, $keyword, $scriptlog_imgthumb, $date_published);
            $theme_meta['site_meta_tags'] = (is_null($tag_item)) ? generate_meta_tags(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_meta_tags('Tag: ' . ucfirst(trim($tag_item)), $description, app_info()['site_name'], $scriptlog_image, $canonical);

            break;

        default:
            $theme_meta['site_schema'] = generate_schema_org(app_info()['site_name'], app_url(), $scriptlog_image, app_info()['site_description'], escape_html(app_info()['site_keywords']), $scriptlog_imgthumb, date(DATE_ATOM));
            $theme_meta['site_meta_tags'] = generate_meta_tags(escape_html(app_info()['site_description']), escape_html(app_info()['site_keywords']), escape_html(app_info()['site_name']), $scriptlog_image, app_url());
...
function metatag_by_query($key, $value, $scriptlog_image, $scriptlog_imgthumb)
{
    ...
    switch ($key) {
        case 'p':
            ...
            $theme_meta['site_schema'] = (is_null($post_id)) ? generate_schema_org(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : BlogSchema::generateBlogPostSchema($post_id);
            $theme_meta['site_meta_tags'] = (is_null($post_id)) ? generate_meta_tags(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_meta_tags($post_title, $description, $author, $image, $canonical);
        ...
        case 'pg':
            if ((empty($value)) || ($value === '')) {
                ...
                $page_title = (!empty($read_page['post_title'])) ? escape_html($read_page['post_title']) : "";
                $description = (!empty($read_page['post_summary'])) ? escape_html($read_page['post_summary']) : escape_html($read_page['post_title']);
                $author = (!empty($read_page['user_login'])) ? escape_html($read_page['user_login']) : escape_html(app_info()['site_name']);
            ...
            $theme_meta['site_schema'] = (is_null($page_id)) ? generate_schema_org(ucfirst(trim('page not found')) . ' | ' . escape_html(app_info()['site_name'])) : generate_schema_org(ucfirst(trim($page_title)) . ' | ' . app_info()['site_name'], $canonical, $image, $description, escape_html(app_info()['site_keywords']), $scriptlog_imgthumb, $date_published, $date_modified);
            $theme_meta['site_meta_tags'] = (is_null($page_id)) ? generate_meta_tags(ucfirst(trim('page not found')) . ' | ' . app_info()['site_name']) : generate_meta_tags($page_title, $description, $author, $image, $canonical);
        ...
        case 'cat':
            if ((empty($value)) || ($value === '')) {
                ...
                $description = isset($read_category['topic_title']) ? escape_html($read_category['topic_title']) : app_info()['site_description'];
        ...
        case 'a':
            ...
            $theme_meta['site_meta_tags'] = (is_null($month)) ? generate_meta_tags(ucfirst(trim('page not found')), app_info()['site_description'], app_info()['site_keywords'], APP_TITLE, $scriptlog_image, app_url()) : generate_meta_tags($month_name, app_info()['site_description'], app_info()['site_name'], $scriptlog_image, $canonical);
        ...
        case 'blog':
            ...
            $theme_meta['site_meta_tags'] = generate_meta_tags('Blog: ' . ucfirst(trim(app_info()['site_name'] . ' | ' . app_info()['site_tagline'])), app_info()['site_description'], app_info()['site_keywords'], $scriptlog_image, app_url() . DS . 'blog');

            break;

        default:
            $theme_meta['site_schema'] = generate_schema_org(app_info()['site_name'], app_url(), $scriptlog_image, app_info()['site_description'], app_info()['site_keywords'], $scriptlog_imgthumb, date(DATE_ATOM));
            $theme_meta['site_meta_tags'] = generate_meta_tags(app_info()['site_description'], app_info()['site_keywords'], app_info()['site_name'], $scriptlog_image, app_url()) ?? "";
