 ...
 * Post view model.
 *
 * Carries the already-escaped, display-ready fields for a single post
 * (home, single, category, tag, blog and search list rendering). Templates
 * print the getters directly; no logic, no queries.
 *
 * @category Theme
 ...
 * @version  1.0
 */
final class PostViewModel extends AbstractThemeViewModel
{
    /**
     * Build a post view model from a prepared row.
     *
    ...
    public static function fromRow(array $row, callable $escape)
    {
        ...
        $self->values = [
            ...
            'author'        => $self::safe($row['user_fullname'] ?? ($row['user_login'] ?? null), $escape),
            ...
            'visibility'    => $self::safe($row['post_visibility'] ?? 'public', $escape),
            'type'          => $self::safe($row['post_type'] ?? 'blog', $escape),
            'comments'      => $self::safe($row['total_comments'] ?? null, $escape),
        ];
    ...
    /**
     * Build a post view model from already-prepared, already-safe values.
     *
     * Used by the theme card pipeline (prepare_post_card()): the values are
     * escaped or sanitized exactly once at the normalization boundary, so they
     * are stored verbatim with no second escaping pass. Fields that are trusted
     * sanitized HTML (content, topics) must be passed as-is and printed as-is.
     *
     * @param array<string, mixed> $prepared Prepared card fields
     * @return static
     *
    ...
    /** @psalm-suppress PossiblyUnusedMethod -- public getter consumed by public/themes */
    public function type(): ?string { return $this->values['type'] ?? null; }
    /** @psalm-suppress PossiblyUnusedMethod -- public getter consumed by public/themes */
    public function comments(): ?string { return $this->values['comments'] ?? null; }
    /** @psalm-suppress PossiblyUnusedMethod -- public getter consumed by public/themes */
    public function topics(): ?string { return $this->values['topics'] ?? null; }
}
