...
class NoCSRF
{
     ...
     * @param Mixed $origin
     *            The object/associative array to retreive the token data from (usually $_POST).
     * @param Boolean $throwException
     *            (Facultative) TRUE to throw exception on check fail, FALSE or default to return false.
     * @param Integer $timespan
     *            (Facultative) Makes the token expire after $timespan seconds. (null = never)
     * @param Boolean $multiple
     *            (Facultative) Makes the token reusable and not one-time. (Useful for ajax-heavy requests).
     *            
     ...
     * After generating the token, put it inside a hidden form field named $key.
     *
     * @param String $key
     *            The session key where the token will be stored. (Will also be the name of the hidden field name)
