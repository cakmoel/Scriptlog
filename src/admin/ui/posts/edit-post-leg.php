...
?>">
            ...
            <div lass="form-group">
               <labe for="visibility">Post visibility</labe>
               <select name="visibility" class="form-control" onchange="checkVisibilitySelection();" id="visibility.system">
                  <?php 
                    $selected = isset($postData['post_visibility']) ? safe_html($postData['post_visibility']) : "";
                    $visibility_list = ['public', 'private', 'protected'];
                    if (!empty($selected)) {
                        
                       foreach ($visibility_list as $values) {
                        
                         $select = $selected  === $visibility_list[2] ? "  selected" : "";

                         echo '<option value="'.$values.'"'.$select.'>'.$values.'</option>'. PHP_EOL;
               ...
               <input type="text" class="form-control" name="post_password" value="" placeholder="Use a secure password">
               <p class="help-block">Protected with a password you choose. Only those with the password can view this post.</p>
               </div>
            </div>
