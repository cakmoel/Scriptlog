...
/**
 * TastyBites Theme Functions
 * 
 * Theme-specific functions for the TastyBites food blog theme.
...
function locale_url(string $path = '', ?string $locale = null): string
{
    ...
    if ($targetLocale === $defaultLocale || $targetLocale === 'en') {
        return $path;
    }
 ...
 * (Required by header.php for <html lang=""> attribute)
 */
if (!function_exists('get_locale')) {
function get_locale(): string
{
    // Try I18nManager first
    if (class_exists('I18nManager')) {
        $i18n = I18nManager::getInstance();
        return $i18n->getLocale();
    }
    
    // Fallback to detect_browser_locale() which checks session/cookie
 ...
 * Matches blog theme implementation for permalink/i18n compatibility
 */
...
function language_switcher(array $args = []): string
{
    ...
    $html .= '<button class="btn btn-outline-primary dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">';
    $html .= '<i class="fa fa-globe" aria-hidden="true"></i> ' . htmlspecialchars($current_native, ENT_QUOTES, 'UTF-8');
    $html .= '</button>';
    ...
    foreach ($locales as $locale) {
        ...
        if (!$permalinksEnabled) {
            ...
            $lang_url = function_exists('locale_url') ? locale_url($_SERVER['REQUEST_URI'] ?? '/', $locale) : '?switch-lang=' . urlencode($locale);
        }
        
        $native_name = get_language_name($locale, true);
        $english_name = get_language_name($locale, false);
        
        $html .= '<a class="dropdown-item ' . $is_active . '" href="' . htmlspecialchars($lang_url, ENT_QUOTES, 'UTF-8') . '"';
        $html .= ' role="menuitem"';
        $html .= ' aria-label="' . htmlspecialchars($english_name, ENT_QUOTES, 'UTF-8') . '"';
        $html .= ' aria-current="' . ($is_active === 'active' ? 'page' : 'false') . '">';
        $html .= '<span class="lang-flag" aria-hidden="true">';
        if ($is_active === 'active') {
            $html .= '<i class="fa fa-check text-success" aria-hidden="true"></i>';
        } else {
            $html .= '<i class="fa fa-circle-o text-muted" aria-hidden="true"></i>';
        }
        $html .= '</span>';
        $html .= '<span class="lang-info">';
        $html .= '<span class="lang-native">' . htmlspecialchars($native_name, ENT_QUOTES, 'UTF-8') . '</span>';
        $html .= '<span class="lang-english">' . htmlspecialchars($english_name, ENT_QUOTES, 'UTF-8') . '</span>';
        $html .= '</span>';
        $html .= '</a>';
    }
...
function previous_post($id)
{
    ...
    while ($rows = $stmt->fetch()) {
        ...
        $html .= '<h6>' . htmlspecialchars($rows['post_title'], ENT_QUOTES, 'UTF-8') . '</h6>';
        $html .= '</div>';
        $html .= '</a>';
    }
...
function render_comments_section($postId)
{
    $total = total_comment($postId)['total'] ?? 0;
    
    ob_start(); ?>
    <div class="comments-section">
        <h4 class="comments-title">
            Comments <span class="badge badge-primary"><?= (int)$total ?></span>
        </h4>
        <div id="comments-list" data-post-id="<?= (int)$postId ?>">
            <!-- Comments loaded via AJAX -->
        </div>
        <div class="text-center mt-3">
            <button class="btn btn-outline-secondary load-comments-btn" data-post-id="<?= (int)$postId ?>">
                Load Comments
            </button>
        </div>
    </div>
...
/**
 * tastybites_make_date() - Format date for display
 */
if (!function_exists('tastybites_make_date')) {
function tastybites_make_date(string $timestamp): string
{
    if (empty($timestamp)) {
        return '';
    }
...
/**
 * tastybites_htmlout() - Escape HTML output
 */
if (!function_exists('tastybites_htmlout')) {
function tastybites_htmlout(string $string): string
{
...
function get_download_page_data($identifier)
{
    ...
    if (!class_exists('DownloadController') || !class_exists('DownloadService')
        || !class_exists('DownloadModel') || !class_exists('MediaDao')) {
        return ['error' => 'Download system not available'];
    }

    try {
        $downloadController = new DownloadController(
            new DownloadService(new DownloadModel(), new MediaDao())
        );
