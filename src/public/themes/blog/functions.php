...
if (!function_exists('get_post_thumbnail')) {
    function get_post_thumbnail($post_img, $post_title, $img_alt = '')
    {
        if (empty($post_img) || $post_img === 'NULL') {
            ...
            return '<img src="' . app_url() . '/public/files/pictures/nophoto.jpg" alt="' . htmlout($post_title) . '" width="730" height="486" class="img-fluid" loading="lazy" decoding="async">';
        }
        ...
        if ($image_check !== false) {
            // Image exists - use responsive image
            return invoke_responsive_image($post_img, 'medium', true, !empty($img_alt) ? $img_alt : $post_title, 'img-fluid', false, 'lazy');
        } else {
